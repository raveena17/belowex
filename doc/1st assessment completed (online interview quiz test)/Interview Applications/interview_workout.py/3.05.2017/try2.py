...
class Attendee:
    ...
    def start(self):
            for qus in self.questions[self.current_question]['QUS'].iteritems():
                valid = True
                print qus['QUS'] 
                for opt, ans in self.questions[self.current_question]['OPTIONS'].iteritems():
                    print opt + ':' + ans
                while valid:
                    ans = raw_input("Choose your answer: ")
                    if ans in self.questions[self.current_question]['OPTIONS'].keys():
                        valid = False
                    elif ans == "":
                        valid = False
                    else:
                        tkMessageBox.showinfo(title="INVALID OPTION", message="Plz check Your option!")
                ans = raw_input("Enter your Ans: ")
                if ans == self.questions[self.current_question]['ANS']:
                        self.correct_answers.append(self.current_question)
                elif ans == "N":
                        self.skiped_questions.append(self.current_question)
                else:
